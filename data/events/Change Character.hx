...
public function changeCharacter(strumIndex:Int, charName:String = 'bf', memberIndex:Int = 0, ?updateBar:Bool = true) {
	...
	if (oldChar.curCharacter == newChar.curCharacter) return; //trace('It\'s the same character bro.'); // ok
	...
	if (memberIndex == 0 && updateBar) {
		if (strumIndex == 0) { // opponent side
    		...
    		iconP2.origin.set(iconP2.width, iconP2.height / 2); // i like psych engines icon bop
